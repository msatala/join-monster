...
class AliasNamespace {
  constructor(minify) {
    this.minify = !!minify; // a generator for infinite alias names, starting with the shortest possible
    // this is helpful for generating the names when minifying

    this.mininym = _generatorics.default.baseNAll('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ#$'); // keep track of all the table names we've used since these have to be unique in each query

    this.usedTableAliases = new Set(); // we can re-use aliases for columns since columns names dont need to be globally unique because they will be prefixed
  ...
  generate(type, name) {
    ...
    name = name.replace(/\s+/g, '').replace(/[^a-zA-Z0-9]/g, '_').slice(0, 10); // the table aliases must be unique
...
//# sourceMappingURL=alias-namespace.js.map