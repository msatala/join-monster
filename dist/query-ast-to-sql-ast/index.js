...
function queryASTToSqlAST(resolveInfo, options, context) {
  ...
  const namespace = new _aliasNamespace.default(options.dialect === 'oracle' ? true : options.minify); // we'll build up the AST representing the SQL recursively

  const sqlAST = {}; // v0.8 changed the "fieldASTs" property to "fieldNodes". we want to support both

  let fieldNodes = resolveInfo.fieldNodes || resolveInfo.fieldASTs; // fieldNodes is usually an array of 1 GraphQL node. If a field is requested twice *without* aliases, both nodes will be in this array
  // we need to merge it into one

  fieldNodes = mergeAll(fieldNodes);

  _assert.default.equal(fieldNodes.length, 1, 'We thought this would always have a length of 1. FIX ME!!'); // this represents the parsed query


  const queryAST = fieldNodes[0]; // resolveInfo.parentType is from the schema, its the GraphQLObjectType that is parent to the current field
  ...
  populateASTNode.call(resolveInfo, queryAST, parentType, sqlAST, namespace, 0, options, context); // make sure they started this party on a table, interface or union.

  _assert.default.ok(['table', 'union'].indexOf(sqlAST.type) > -1, 'Must call joinMonster in a resolver on a field where the type is decorated with "sqlTable".'); // make sure each "sqlDep" is only specified once at each level. also assign it an alias
...
function populateASTNode(queryASTNode, parentTypeNode, sqlASTNode, namespace, depth, options, context) {
  ...
  const fieldName = queryASTNode.name.value; // if this is an internal field (say, for introspection "__typename"), lets ignore it
  ...
  let grabMany = false; // the actual type might be wrapped in a GraphQLNonNull type

  let gqlType = stripNonNullType(field.type);
  sqlASTNode.args = (0, _values.getArgumentValues)(field, queryASTNode, this.variableValues); // if list then mark flag true & get the type inside the GraphQLList container type
  ...
  if (gqlType.constructor.name === 'GraphQLObjectType' && gqlType._fields.edges && gqlType._fields.pageInfo) {
    grabMany = true; // grab the types and fields inside the connection

    const stripped = stripRelayConnection(gqlType, queryASTNode, this.fragments); // reassign those

    gqlType = stripNonNullType(stripped.gqlType);
    queryASTNode = stripped.queryASTNode; // we'll set a flag for pagination.
  ...
  const config = gqlType._typeConfig; // is this a table in SQL?

  if (!field.jmIgnoreTable && TABLE_TYPES.includes(gqlType.constructor.name) && config.sqlTable) {
    if (depth >= 1) {
      (0, _assert.default)(!field.junctionTable, '"junctionTable" has been replaced with a new API.');
      (0, _assert.default)(field.sqlJoin || field.sqlBatch || field.junction, `If an Object type maps to a SQL table and has a child which is another Object type that also maps to a SQL table,
    ...
    handleTable.call(this, sqlASTNode, queryASTNode, field, gqlType, namespace, grabMany, depth, options, context); // is this a computed column from a raw expression?
    ...
    sqlASTNode.as = namespace.generate('column', aliasFrom); // is it just a column? if they specified a sqlColumn or they didn't define a resolver, yeah
    ...
    sqlASTNode.as = namespace.generate('column', aliasFrom); // or maybe it just depends on some SQL columns
  } else if (field.sqlDeps) {
    sqlASTNode.type = 'columnDeps';
    sqlASTNode.names = field.sqlDeps; // maybe this node wants no business with your SQL, because it has its own resolver
  } else {
    sqlASTNode.type = 'noop';
  }
}
...
function handleTable(sqlASTNode, queryASTNode, field, gqlType, namespace, grabMany, depth, options, context) {
  ...
  sqlASTNode.name = sqlTable; // the graphQL field name will be the default alias for the table
  ...
  if (field.sqlJoin) {
    sqlASTNode.sqlJoin = field.sqlJoin; // or a many-to-many?
    ...
    if (field.junction.sqlJoins) {
      ...
      children.push({ ...keyToASTChild((0, _util.ensure)(field.junction, 'uniqueKey'), namespace),
        fromOtherTable: junction.as
      });
      junction.sqlBatch = {
        sqlJoin: (0, _util.ensure)(field.junction.sqlBatch, 'sqlJoin'),
        thisKey: { ...columnToASTChild((0, _util.ensure)(field.junction.sqlBatch, 'thisKey'), namespace),
          fromOtherTable: junction.as
        },
  ...
  if (field.limit) {
    (0, _assert.default)(field.orderBy, '`orderBy` is required with `limit`');
...
function handleUnionSelections(sqlASTNode, children, selections, gqlType, namespace, depth, options, context, internalOptions = {}) {
  for (let selection of selections) {
    ...
    switch (selection.kind) {
      ...
      case 'InlineFragment':
        {
          const selectionNameOfType = selection.typeCondition.name.value; // normally, we would scan for the extra join-monster data on the current gqlType.
  ...
  for (let selection of selections) {
    ...
    switch (selection.kind) {
      ...
      case 'FragmentSpread':
        {
          const fragmentName = selection.name.value;
          const fragment = this.fragments[fragmentName]; // make sure fragment type (or one of the interfaces it implements) matches the type being queried
...
function handleColumnsRequiredForPagination(sqlASTNode, namespace) {
  ...
  if (sqlASTNode.sortKey || ((_ref4 = sqlASTNode) != null ? (_ref4 = _ref4.junction) != null ? _ref4.sortKey : _ref4 : _ref4)) {
    ...
    (0, _assert.default)(sortKey.order, '"sortKey" must have "order"'); // this type of paging uses the "sort key(s)". we need to get this in order to generate the cursor

    for (let column of (0, _util.wrap)((0, _util.ensure)(sortKey, 'key'))) {
      const newChild = columnToASTChild(column, namespace); // if this joining on a "through-table", the sort key is on the threw table instead of this node's parent table
...
function stripRelayConnection(gqlType, queryASTNode, fragments) {
  ...
  const strippedType = stripNonNullType(stripNonNullType(edgeType.ofType)._fields.node.type); // let's remember those arguments on the connection

  const args = queryASTNode.arguments; // and then find the fields being selected on the underlying type, also buried within edges and Node
...
function pruneDuplicateSqlDeps(sqlAST, namespace) {
  ...
  for (let children of childrenToLoopOver) {
    ...
    // a junction table.
    const depsByTable = {}; // loop thru each child which has "columnDeps", remove it from the tree, and add it to the set
    ...
    for (let i = children.length - 1; i >= 0; i--) {
      ...
      if (child.type === 'columnDeps') {
        ...
        children.splice(i, 1); // or if its another table, recurse on it
