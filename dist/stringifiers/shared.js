...
function interpretForOffsetPaging(node, dialect) {
  ...
  if ((_ref5 = node) != null ? (_ref5 = _ref5.args) != null ? _ref5.first : _ref5 : _ref5) {
    limit = parseInt(node.args.first, 10); // we'll get one extra item (hence the +1). this is to determine if there is a next page or not
...
function interpretForKeysetPaging(node, dialect) {
  ...
  if (node.sortKey) {
    ...
    sortTable = node.as; // flip the sort order if doing backwards paging
    ...
    descending = sortKey.order.toUpperCase() === 'DESC';
    sortTable = node.junction.as; // flip the sort order if doing backwards paging
